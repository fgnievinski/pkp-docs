...
The Model classes are PHP classes responsible only for representing database entities in memory. For example, the ``articles`` table stores article information in the database; there is a corresponding Model class called ``Article`` (see ``classes/article/Article.inc.php``) and DAO class called ``ArticleDAO`` (see the section called [Data Access Objects](https://pkp.sfu.ca/ojs/docs/technicalreference/2.1/classReferenceDataAccessObjects.html) [DAOs]).
